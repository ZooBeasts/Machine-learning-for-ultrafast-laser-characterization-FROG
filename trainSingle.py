...
from Dataholder import single_data, transforms
...
save_dir = 'logs_shg_re_2/'
os.makedirs(save_dir) if not os.path.exists(save_dir) else print("Directory exists")

training_set = single_data('Ere_20k.csv',
                           'SHG/',
                           transforms=transforms,
                           train=True)

test_set = single_data('Ere_20k.csv',
                       'SHG/',
...
# model = resnet9_with_attention(3, 64, num_output).to(device)
model = resnet9(3, 64, num_output).to(device)
